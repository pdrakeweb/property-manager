          ...
          <div className="space-y-2">
            {MODELS_BY_TASK.map(({ task, default: def }) => (
              <div key={task} className="flex items-center justify-between gap-2">
                <span className="text-xs text-slate-600 truncate flex-1">{task}</span>
                <select
                  defaultValue={def}
                  className="text-xs border border-slate-200 rounded-lg px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-sky-300 shrink-0 max-w-[180px]"
                >
