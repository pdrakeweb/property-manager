...
import {
  ...
  Clock, SkipForward, Undo2, CheckSquare, Sparkles,
} from 'lucide-react'
...
import { getCustomItems } from '../lib/checklistCustomStore'
      ...
      <div className="flex items-start gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {item.source === 'ai' && (
              <span
                title="Generated by AI from your property context"
                className="inline-flex items-center gap-0.5 text-[10px] font-semibold uppercase tracking-wide bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300 px-1.5 py-0.5 rounded"
              >
...
export function ChecklistRunScreen() {
  ...
  // Pool = baseline items + any AI/user custom items saved for this property+template.
  // The run snapshot (run.items) is the source of truth for what's *in* this run,
  // but we need full definitions (label/detail/etc.) from somewhere.
  const itemPool: ChecklistItem[] = useMemo(() => {
    const baseline = (template?.items ?? []).map(i => ({
      ...i,
      source: i.source ?? ('baseline' as const),
    }))
    const custom = getCustomItems(run.propertyId, run.templateId)
    return [...baseline, ...custom]
  }, [template, run.propertyId, run.templateId])

  // Only include items that are in this run
  const runItemIds = new Set(run.items.map(ri => ri.itemId))
  const activeItems = itemPool.filter(i => runItemIds.has(i.id))
  ...
  const headerLabel = run!.kind === 'adhoc'
    ? (run!.name ?? template?.name ?? 'Checklist')
    : (SEASON_LABELS[run!.season ?? ''] ?? run!.season ?? 'Checklist') + ' Checklist'
