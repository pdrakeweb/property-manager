...
test('seeds DEFAULT_PROPERTIES on first load', async ({ page }) => {
  ...
  const stored = await page.evaluate(() =>
    JSON.parse(localStorage.getItem('pm_properties_v1') ?? '[]') as Array<{ id: string }>,
  )
...
test('Settings page lists every property from the store', async ({ page }) => {
  ...
  const stored = await page.evaluate(() =>
    JSON.parse(localStorage.getItem('pm_properties_v1') ?? '[]') as Array<{ name: string }>,
  )
...
test('adding a property via store updates the Settings list', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const list = JSON.parse(localStorage.getItem('pm_properties_v1') ?? '[]') as Array<Record<string, unknown>>
    list.push({
      id: 'pt_test_cabin',
    ...
    localStorage.setItem('pm_properties_v1', JSON.stringify(list))
  })
